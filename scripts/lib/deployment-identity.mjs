...
export async function bindVercelCanonicalAlias({ deploymentUrl, candidates, aliasFn = spawnVercelAliasSet, rootDir, maxAttemptsPerAlias = aliasFn === spawnVercelAliasSet ? 4 : 1 } = {}) {
	...
	for (const candidate of aliases) {
		for (let attempt = 1; attempt <= maxAttemptsPerAlias; attempt += 1) {
			...
			if (attempt < maxAttemptsPerAlias) await new Promise((resolve) => setTimeout(resolve, 5000));
		}
	}
